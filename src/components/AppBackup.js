...
const Menu = ({ children }) => {
  return <ul className="bg-violet-300 md:bg-white h-16 md:min-h-20 list-none p-0 m-0 flex items-stretch ">{children}</ul>;
};

const MenuMobile = ({ children }) => {
  const [burgerOpen, setBurgerOpen] = useState(false);

  let hamburger_menu_icon = burgerOpen ? closeHamburgerMenuIcon : openHamburgerMenuIcon;
  ...
  return (
    <ul className="bg-violet-300 h-16 list-none p-0 m-0 flex items-stretch">
      {children}
      <img
        onClick={handleOnClick}
        src={hamburger_menu_icon}
        alt={T.translate("hamburgerMenuIcon")}
        className="ml-auto my-auto h-5 w-5 p-3 box-content bg-slate-400 mr-2.5"
      />
    </ul>
  );
};
      ...
      <MenuMobile>
        <MenuItem text="It's mine" url="/">
          <img
            src={siteLogo}
            alt={T.translate("siteLogo")}
            className="h-8 md:h-10 w-8 md:w-10 mr-2.5 rounded-full border-2 border-primary"
          />
          {T.translate("siteName")}
        </MenuItem>
        {/* <hr className="bg-menu-separator w-px h-16 p-0 ml-auto mt-auto mb-auto" /> */}

        {/* <MenuItem text="menu icon" url="/" alignR>
          <img
            src={openHamburgerMenuIcon}
            alt={T.translate("hamburgerMenuIcon")}
            className="ml-auto h-5 w-5 p-3 box-content bg-slate-400 mr-2.5"
          />
          <img
            src={closeHamburgerMenuIcon}
            alt={T.translate("closeHamburgerMenuIcon")}
            className="ml-auto h-5 w-5 p-3 box-content bg-slate-400 mr-2.5"
          />
        </MenuItem> */}
      </MenuMobile>
